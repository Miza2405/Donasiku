    ...
    <div id="content">
        ...
        <div id="sec-dashboard" class="content-section active-section">
            <div class="row g-4 mb-4">
                <div class="col-md-4"><div class="card dash-card p-4"><div class="d-flex justify-content-between align-items-center"><div><p class="text-muted fw-semibold mb-1">Total Dana Donasi</p><h3 class="fw-bold text-emerald mb-0">Rp 120.5M</h3></div><div class="icon-box bg-success bg-opacity-10 text-success"><i class="bi bi-cash-stack"></i></div></div></div></div>
                <div class="col-md-4"><div class="card dash-card p-4"><div class="d-flex justify-content-between align-items-center"><div><p class="text-muted fw-semibold mb-1">Menunggu Verifikasi</p><h3 class="fw-bold text-warning mb-0">15 Trx</h3></div><div class="icon-box bg-warning bg-opacity-10 text-warning"><i class="bi bi-hourglass-split"></i></div></div></div></div>
                <div class="col-md-4"><div class="card dash-card p-4"><div class="d-flex justify-content-between align-items-center"><div><p class="text-muted fw-semibold mb-1">Total Donatur</p><h3 class="fw-bold text-primary mb-0">1,245 Orang</h3></div><div class="icon-box bg-primary bg-opacity-10 text-primary"><i class="bi bi-people-fill"></i></div></div></div></div>
            </div>
        ...
        <div id="sec-donatur" class="content-section">
            ...
            <div class="table-custom table-responsive">
                <table class="table table-hover mb-0">
                    ...
                    <tbody><tr><td>1</td><td class="fw-bold">Suud Nofa </td><td>SuudNofa@email.com</td><td>081234567890</td><td>10 Jan 2026</td><td><button class="btn btn-sm btn-outline-danger"><i class="bi bi-trash"></i></button></td></tr></tbody>
                </table>
            </div>
        </div>
        ...
        <div id="sec-transaksi" class="content-section">
            ...
            <div class="table-custom table-responsive">
                <table class="table table-hover mb-0">
                    ...
                    <tbody><tr><td>20 Mar 2026</td><td class="fw-bold">#TRX-003</td><td>Budi Santoso</td><td>Santunan Yatim</td><td class="fw-semibold text-emerald">Rp 500.000</td><td><span class="badge bg-success">Berhasil</span></td></tr></tbody>
                </table>
            </div>
        </div>
...
<script>
    ...
    function aksiVerifikasi() { 
        ...
        Swal.fire({ icon: 'success', title: 'Transaksi Diverifikasi!', text: 'Dana telah tercatat ke dalam sistem.', confirmButtonColor: '#059669' }).then(() => {
            let badge = document.getElementById('badge-' + currentTrxId);
            if (badge) { badge.className = 'badge bg-success'; badge.innerText = 'Berhasil'; }
            let aksiTd = document.getElementById('aksi-' + currentTrxId);
            if (aksiTd) { aksiTd.innerHTML = '<span class="text-muted small fw-bold"><i class="bi bi-check2-all text-success"></i> Selesai</span>'; }
        }); 
    }

    function aksiTolak() { 
        bootstrap.Modal.getInstance(document.getElementById('invoiceModal')).hide(); 
        Swal.fire({ icon: 'error', title: 'Transaksi Ditolak', text: 'Transaksi telah dibatalkan.', confirmButtonColor: '#d33' }).then(() => {
            let badge = document.getElementById('badge-' + currentTrxId);
            if (badge) { badge.className = 'badge bg-danger'; badge.innerText = 'Ditolak'; }
            let aksiTd = document.getElementById('aksi-' + currentTrxId);
            if (aksiTd) { aksiTd.innerHTML = '<span class="text-muted small fw-bold"><i class="bi bi-x-circle text-danger"></i> Dibatalkan</span>'; }
        }); 
    }
    ...
    function simpanProgram() {
        ...
        Swal.fire({
            icon: 'success', title: 'Program Diterbitkan!',
            text: 'Program kampanye baru berhasil ditambahkan dan sudah tampil di halaman donasi utama.',
            confirmButtonColor: '#059669'
        });
        document.getElementById('formProgramBaru').reset(); // Bersihkan form
    }
